...
from network.ops import get_knn_feats, spectral_smooth, trans, compute_smooth_motion_diff, spectral_smooth_yuan
...
class CRLayer(nn.Module):
    ...
    def forward(self, feats, eig_vec, eig_val,):
        feats_smooth=spectral_smooth_yuan(feats,eig_vec,eig_val,self.eta)
...
class LMCNet(nn.Module):
    ...
    def forward(self, data):
        # print(data.keys())
        # print("1111", data['eig_vec'].shape, data['eig_val'].shape, data['xs'].shape)
        ...
        # YYYY torch.Size([16, 1, 1897, 4]) torch.Size([16, 1897])
        # YYYY torch.Size([16, 4, 2000, 1]) torch.Size([16, 1, 2000, 1])
        # print("YYYY", xs.shape, logits.shape)
        ...
        return res_logits, res_e_hat, 0, 0, 0
...
def batch_symeig_batch(X):
    e, v = torch.linalg.eigh(X, UPLO='U')
    return v
